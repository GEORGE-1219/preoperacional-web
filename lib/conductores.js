...
let conductoresSchemaEnsured = false;
...
export async function ensureConductoresSchema() {
  if (conductoresSchemaEnsured) return;
...
export async function listConductoresBasicos({ includeInactive = false } = {}) {
  await ensureConductoresSchema();

  return query(
    `SELECT id, documento, nombre, activo
       FROM usuarios_autorizados
      WHERE (:includeInactive = 1 OR activo = 1)
      ORDER BY nombre ASC
      LIMIT 1000`,
